...
complete3 <- complete3[,c(6,8,10,11,12,13,14,15,16,18)]
...
```{r}
boxcox(complete3model, lambda = seq(-1, 2, .1))
```
### The BoxCox Lambda Gets Even Worse 

### We Proceed To Run A Cook's Distance Test To Test For Influential Observations 
```{r}
cook2 <- cooks.distance(complete3model)
halfnorm(cook2, 1, ylab = "Cooks's Distances")
outlierTest(complete3model)
```
### Our outliers are significant in the model so we cannot remove them
